...
glibr::glibr( PinName sda, PinName scl ) :
		i2c( sda, scl )
{
	gesture_ud_delta_ = 0;
	gesture_lr_delta_ = 0;
...
//#endif
			...
			if ( fifo_level > 0 )
			{
				check = I2CReadDataBlock( APDS9960_I2C_ADDR, APDS9960_GFIFO_U, fptr, (fifo_level * 4) );
...
bool glibr::setLEDBoost( uint8_t boost )
{
	...
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_CONFIG2, val ) )
...
bool glibr::setProxGainCompEnable( uint8_t enable )
{
	...
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_CONFIG3, val ) )
...
bool glibr::setGestureEnterThresh( uint8_t threshold )
{
	...
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_GPENTH, threshold ) )
	{
...
{
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_GEXTH, threshold ) )
...
bool glibr::setGestureGain( uint8_t gain )
{
	...
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_GCONF2, val ) )
...
bool glibr::setLightIntHighThreshold( uint16_t threshold )
{
	...
	/* Write low byte */
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_AIHTL, val_low ) )
	{ //!wireWriteDataByte(APDS9960_AIHTL, val_low) ) {
		return false;
	}

	/* Write high byte */
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_AIHTH, val_high ) )
...
bool glibr::setAmbientLightIntEnable( uint8_t enable )
{
	...
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_ENABLE, val ) )
...
bool glibr::setGestureIntEnable( uint8_t enable )
{
	...
	if ( I2CwriteByte( APDS9960_I2C_ADDR, APDS9960_GCONF4, val ) )
